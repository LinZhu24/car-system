package com.lxr.carsystem.repo;

import com.lxr.carsystem.entity.AppointLook;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.JpaSpecificationExecutor;

import java.util.List;

public interface AppointLookRepo extends JpaSpecificationExecutor<AppointLook>,JpaRepository<AppointLook,Integer>{
    AppointLook findByUserid(Integer userid);
    List<AppointLook> findAllByUserid(Integer userid);

    AppointLook findByAppointlookid(Integer aptlkid);

    AppointLook findByWscid(Integer wscid);
}